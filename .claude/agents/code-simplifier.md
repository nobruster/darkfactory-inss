...
### Nesta fábrica, jamais toque em (mesmo parecendo redundante)

| Código | Por quê |
|---|---|
| `Decimal(...)` em caminho monetário | ADR 0002 — `float` é gate reprovado, não estilo |
| Publicação em `.parcial` + `rename` | é o "zero artefato quando reprova"; colapsar isso publica lixo |
| Gate que compara `count`/`sum` | tolerância zero é a doutrina, não verbosidade |
| Leitura posicional (`p[12]`, `p[13]`) | DF-INSS-001 — ler por nome perde o código em silêncio |
| `especie_nome_fonte` preservado junto do oficial | DF-INSS-004 — apagar destrói a evidência da divergência |
| `if confere_controle:` antes de comparar totais | competência nova tem outros números; não é código morto |

**Duplicação que protege não é duplicação.** Se duas verificações parecem
redundantes, leia o ADR antes de propor fundir — provavelmente elas respondem
